...
Pirkejai pirkejai = new Pirkejai();
		...
		if ( prideti.equals( "prideti" ) ) {
			...
			Integer miesto_id = Integer.parseInt(miestas);
			
			if (miesto_id > 0) {
				
				Optional <Miestai> found = miestai_repository.findById( miesto_id );
				
				if ( found.isPresent() ) {
					
					Miestai gyv_miestas = found.get();
					pirkejai.setMiestai(gyv_miestas);
